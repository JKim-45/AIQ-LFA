...
## Before running, you can revise the following parameters by editing params.csv file; 
	...
	ran_seed - random seed for classification of training set / test set (in this manuscript, 42)
...
## When you run the code, then new folder will be created and data files will be generated in that folder;  
	...
	AUC_test_~~.xlsx : AUC values with test set  
	...
	confusion_matrix_fpr_~~.xlsx : confusion matrix of test set with cutoff from FPR < FPR_rate  
	confusion_matrix_tpr_~~.xlsx : confusion matrix of test set with cutoff from TPR > TPR_rate  
	P values (Status from P).xlsx : P value from DeLong test of ROC curves from training set and test set, in each repetition  
	Delong.xlsx : DeLong test of ROC curves from training set and test set, and average P value  
	ROC_test_~~.xlsx : representative ROC curve from test set  
	ROC_train_~~.xlsx : representative ROC curve from training set  
	test_for_~~.xlsx : test set  
